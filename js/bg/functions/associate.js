...
function startAssociationRun() {
    0 < associationConcurrentRuns || (fixBadNodes(), chrome.tabs.query({}, function(b) {
        if (!(0 < associationConcurrentRuns)) {
            ...
            for (var f in b) {
                var d = b[f];
                sidebarHandler.tabId == d.id || getUrl(d) == chrome.extension.getURL("/sidebar.html") || tree.getNode(["chromeId", d.id]) || tryAssociateTab(c, d) || c.total++
            }
...
function tryAssociateTab(b, a, c) {
   ...
   // if (a.incognito) return !1;
    if (!c && isNewTabUrl(getUrl(a)) && 0 == a.index && !a.pinned) return chrome.tabs.query({
        windowId: a.windowId
    }, function(c) {
        1 == c.length ? tree.addTabToWindow(a, new PageNode(a)) : tryAssociateTab(b, a, !0)
    }), !1;
    if (tryFastAssociateTab(a, !0)) return !0;
    if (isNewTabUrl(getUrl(a))) return tree.addTabToWindow(a, new PageNode(a)), !0;
    if (!isScriptableUrl(getUrl(a))) return log("Doing blind association for non scriptable tab", a.id, getUrl(a)), associateTabToPageNode(f, a), !0;
    b.tabIds.push(a.id);
    getPageDetails(a.id, {
        action: "associate",
        runId: f
...
function tryFastAssociateTab(b, a) {
    var c = tree.getNode(["chromeId", b.windowId]),
        c = tree.filter(function(c) {
            return c instanceof PageNode && c.hibernated && (!a || c.restorable) && getUrl(b) == getUrl(c) && b.index == c.index && b.incognito == c.incognito && b.pinned == c.pinned
        }, c ? c.children : void 0);
    if (1 == c.length || 0 < c.length && !isScriptableUrl(getUrl(b))) return c = c[0], log("doing fast associate", b, c, b.id, c.id, "pinned states", b.pinned.toString(), c.pinned.toString()), restoreAssociatedPage(b, c), isScriptableUrl(getUrl(b)) && getPageDetails(b.id, {
        action: "store"
    }), c
}
...
function associateTabToPageNode(b, a, c, f) {
    log("Associating tab", "runId", b, "tabId", a.id, "url", getUrl(a), "referrer", c, "historylength", f, "associationRuns", associationRuns);
...
function restoreAssociatedPage(b, a) {
    log("restoring associated page", "tab", b.id, b, "page", a.id, a, getUrl(b), getUrl(a));
    tree.updateNode(a, {
        restored: !0,
        hibernated: !1,
        restorable: !1,
...
function findPageNodeForAssociation(b, a) {
    function c(a) {
        ...
        var c = !1;
        if (g) {
            if (!isGoogleSearchUrl(getUrl(a))) return !1;
            c = getGoogleTestUrl(getUrl(a)) == e
        } else c = d == getUrl(a);
...
function disambiguatePageNodesByWindowId(b) {
    ...
    var c = tree.groupBy(function(a) {
            if (a.isTab()) {
                var c = a.topParent();
                return !c || a.windowId == c.chromeId ? void 0 : [getUrl(a), a.referrer, a.historylength, a.pinned, a.incognito].join("|")
            }
        }),
        f;
    for (f in c) {
...
function swapPageNodesByIndex() {
    var b = tree.groupBy(function(a) {
            if (a.isTab()) {
                var c = a.topParent();
                return !c ? void 0 : [c.id, getUrl(a), a.referrer, a.historylength, a.pinned, a.incognito].join("|")
            }
        }),
        a;
    for (a in b) {
    ...
    tree.rebuildPageNodeWindowIds(function() {
        chrome.tabs.query({}, function(a) {
            a.forEach(function(a) {
                if (getUrl(a) != chrome.extension.getURL("/sidebar.html")) {
