...
DataTreeNode.prototype = {
    ...
    parents: function() {
        for (var a = [], c = this.parent; c; )
            a.push(c),
            c = c.parent;
        return a
    },
    topParent: function() {
        for (var a = this; a.parent && !a.parent.isRoot; )
            a = a.parent;
    ...
    afterSibling: function() {
        var a = this.siblingIndex() + 1;
		try{
			if (!(a >= this.parent.children.length)){
				return this.parent.children[a];
			}
		}catch(e){
			return null;
    ...
    preceding: function(a, c) {
        ...
        if (0 != b.length) {
            if (!a)
                return b[b.length - 1];
            if (b = first(b.reverse(), function(b) {
                return a(b)
            }))
                return b[1]
        }
