...
function onLoad()
{
    ...
    chrome.tabs.getCurrent(async function() {
        ...
        ghostTree = new UiDataTree(
            ...
            undefined,
            function() {
                savePageTree(ghostTree, 'ghostTree');
            },
...
async function savePageTree(tree, settingName) {
	
tree=wipeNodes(tree); 

	/*
	if(tree.name=='tree'||tree.name=='pageTree'){
	initTree=initree();
	console.log(initTree);
	console.log(tree);
...
/*
	...
	chkTree=treeUrls(tree.tree);
	chkInitTree=treeUrls(initTree);
	
	Array.prototype.diff = function(a) {
    return this.filter(function(i) {return a.indexOf(i) < 0;});
};

		console.log(chkTree.diff(chkInitTree));
	*/

			/*
				function setvals(obj,val,rep){
						var k=0;
	var newstr0='';
var newstr1='';
...
while(k=0){
 ...
 newstr1=newstr0.replace(re, rep);
 if(newstr1==newstr0){
 	k=1;
 }else{
 	newstr0=newstr1;
...
/*
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaState":(.*?)\}','"mediaState":null}'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaState":(.*?)\]','"mediaState":null]'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaState":\[(.*?)\]','"mediaState":null'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaState":\{(.*?)\}','"mediaState":null'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaState":(.*?)\,','"mediaState":null,'));
...
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaTime":(.*?)\}','"mediaTime":null}'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaTime":(.*?)\]','"mediaTime":null]'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaTime":\[(.*?)\]','"mediaTime":null'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaTime":\{(.*?)\}','"mediaTime":null'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"mediaTime":(.*?)\,','"mediaTime":null,'));

tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"favicon":(.*?)\}','"favicon":null}'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"favicon":(.*?)\]','"favicon":null]'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"favicon":\[(.*?)\]','"favicon":null'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"favicon":\{(.*?)\}','"favicon":null'));
tree.tree[index]=JSON.parse(setvals(tree.tree[index],'\"favicon":(.*?)\,','"favicon":null,'));
...
function loadPageTreeFromLocalStorage(storedPageTree) {
    ...
    chrome.tabs.query({ }, function(tabs) {
        var urlAndTitles = tabs.map(function(e) { return getUrl(e) + '\n' + e.title });
        ...
        tree.forEach(function(node, index, depth, containingArray, parentNode) {
            ...
            if (node instanceof WindowNode && node.children.length == 1
                ...
                && isNewTabUrl(getUrl(node.children[0]))
            ...
            if (node instanceof PageNode
                && !node.hibernated
                && getUrl(node).match(/^chrome-/)
                && urlAndTitles.indexOf(getUrl(node) + '\n' + node.title) == -1)
            {
...
function PageTreeCallbackProxy(methodName, args) {
    ...
    if (methodName == 'remove' && !node.incognito && (
            ...
            || (getUrl(node) && !getUrl(node).match(/^chrome/))
        )
    ) {
	...
	savePageTree(tree, 'pageTree');
	savePageTree(tree, 'backupPageTree');
}
...
function deduplicateRecentlyClosedPageNode(node) {
    ...
    var dupes = recentlyClosedTree.filter(function(e) {
        return getUrl(e) == getUrl(node) && count++ > 0;
    });
...
function restoreNodeFromRecentlyClosedTree(node, addDescendants, onRestored) {
    ...
    if (!(node instanceof HeaderNode)) {
        ...
        if (!added) {
            ...
            if (!to || to.type != 'normal') {
                to = tree.getNode(function(e) { return e instanceof WindowNode && !e.hibernated && e.type == 'normal'; });
                if (!to) {
                    to = tree.getNode(function(e) { return e instanceof WindowNode  && e.type == 'normal'; });
                    if (!to) {
...
async function shutdownSidewise() {
    ...
    await savePageTree(recentlyClosedTree, 'recentlyClosedTree');
    await savePageTree(ghostTree, 'ghostTree');
