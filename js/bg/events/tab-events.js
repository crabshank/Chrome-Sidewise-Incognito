...
let n_id=pgs.findIndex((p)=>{return p.chromeId===r;});
		...
		if(n_id>=0){
			let nd=pgs[n_id];
			tree.updateNode(nd,{chromeId:a});
		}else{
				let n=tree.getNode(["chromeId", r]); if(typeof n!=='undefined'){
						tree.updateNode(n,{chromeId:a});
				}
		}
...
function onTabCreated(a) {
    ...
    if (!monitorInfo.isDetecting())
        if (sidebarHandler.creatingSidebar && getUrl(a) == sidebarHandler.sidebarUrl) log("ignoring creation of the sidebar");
            ...
            if (b = first(tree.awakeningPages, function(b) {
            ...
            else if (isNewTabUrl(getUrl(a)) && !a.pinned && tree.getWindowIndexedTabsCount(a.windowId) == a.index) {
                b = tree.getNode(["chromeId", a.windowId]).children;
                if (0 < b.length && (b = b[b.length - 1], 0 == b.children.length && b.hibernated && b.restorable && getUrl(b) == getUrl(a) && !b.pinned)) {
                ...
                if (b = new PageNode(a, "preload"), b.unread = !0, b.initialCreation = !1, refreshPageStatus(b), getUrl(a) && 0 == getUrl(a).indexOf("view-source:") && a.openerTabId) tree.addNode(b, ["chromeId", a.openerTabId], void 0, !0);
                else if (getUrl(a) && !isScriptableUrl(getUrl(a))) {
                log("Adding non scriptable tab to tree via association attempt", a.id, a, getUrl(a));
	...
	let pgs=allPageNodes();
		...
		if(n_id>=0){
			...
			if(!nd.hibernated){
				...
				let n=tree.getNode(["chromeId", a]); if(typeof n!=='undefined'){
					if(!n.hibernated){
						tree.removeNode(n);
					}
				}
    ...
    if (a != sidebarHandler.tabId && !monitorInfo.isDetecting()) {
        if (d!==null) {
            ...
            f ? isStaticFavIconUrl(b.favIconUrl) ? e = getBestFavIconUrl(b.favIconUrl, c) : isScriptableUrl(c) ?
                c && getUrl(d) && (c = splitUrl(c), f = splitUrl(getUrl(d)), c && f && (e = c.domain != f.domain ? "chrome://favicon/" : d.favicon)) : e = f ? d.favicon : getBestFavIconUrl("", c) : e = getBestFavIconUrl(b.favIconUrl, c);
