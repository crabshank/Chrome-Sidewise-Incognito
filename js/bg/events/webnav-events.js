...
function onCreatedNavigationTarget(a) {
    if (!(0 < a.frameId || -1 == a.tabId) && !monitorInfo.isDetecting()) {
        ...
        if (b.parent instanceof WindowNode) {
            ...
            c ? c.windowId != b.windowId ? log("Not moving because opener and opened tabs are in different windows", c.windowId, b.windowId, c, b, a) : (log("Moving page to be child of its sourceTabId", a.tabId, a.sourceTabId, a),
            tree.moveNode(b, ["chromeId", a.sourceTabId]),
            tree.conformChromeTabIndexForPageNode(b, !0, !1, !0)) : log("Not moving because could not find sourceTabId " + a.sourceTabId)
        } else
        ...
        if (b && !b.placed)
            if (-1 < a.transitionQualifiers.indexOf("from_address_bar") || "auto_bookmark" == a.transitionType) {
                if (!(b.parent instanceof WindowNode)) {
                    ...
                    if (!a)
                        throw Error("Could not find WindowNode to put page under that was opened via url bar alt-enter");
                    tree.updatePage(b, {
                        placed: !0
                    });
                    tree.moveNode(b, a)
                }
            } else if ("reload" == a.transitionType)
                b.initialCreation && tryAssociateExistingToRestorablePageNode(b);
                ...
                b.placed = !0;
                c ? (c = c[1],
                log("Moving page with link transitionType to be before predicted next-sibling " + c.id, c),
                tree.moveNodeRel(b, "before", c, !0)) : (log("Moving page with link transitionType to be last child of its opener " + a.id),
                tree.moveNodeRel(b, "append", a, !0))
            }
    }
}
function onBeforeNavigate(a) {
    if (!(0 < a.frameId || -1 == a.tabId)) {
        ...
        if (b) {
            ...
            var c = function() {
                chrome.tabs.get(a.tabId, function(c) {
                    ...
                    "complete" == c.status || c.hibernated))
                        ...
                        TimeoutManager.clear("checkPageStatus2_" + a.tabId),
                        TimeoutManager.clear("checkPageStatus3_" + a.tabId)
                })
            };
            ...
            0 < associationConcurrentRuns ? log("Not expecting a tab id swap because associationConcurrentRuns == " + associationConcurrentRuns) : (log("Expecting a tab id swap", a.tabId),
            expectingNavigationTabIdSwap = !0,
            expectingNavigationPossibleNewTabIds.push(a.tabId))
    }
}
function onCompleted(a) {
    if (!(0 < a.frameId || -1 == a.tabId) && !monitorInfo.isDetecting() && void 0 !== tree.getNode(["chromeId", a.tabId]))
        log(a),
        tree.updatePage(a.tabId, {
            placed: !0,
            status: "complete",
            initialCreation: !1
        }),
...
function refreshFaviconAndTitle(a) {
    chrome.tabs.get(a, function(b) {
        if (b) {
            ...
            isStaticFavIconUrl(b.favIconUrl) ? (c = getBestFavIconUrl(b.favIconUrl, c),
            tree.updatePage(a, {
                favicon: c,
                title: getBestPageTitle(b.title, getUrl(b))
            })) : setTimeout(function() {
...
function refreshFaviconTitleLateTimer(a) {
    chrome.tabs.get(a, function(b) {
        if (b) {
            var c = getBestPageTitle(b.title, getUrl(b))
              , d = getUrl(b) ? dropUrlHash(getUrl(b)) : "";
            if (isStaticFavIconUrl(b.favIconUrl))
                tree.updatePage(a, {
                ...
                if (b = splitUrl(d)) {
                    var e = (chrome.extension||window.parent.chrome.extension).getURL("/images/favicon/default.png");
                    setTimeout(function() {
                        tree.updatePage(a, {
                            favicon: e,
                            title: c
                ...
                setTimeout(function() {
                    chrome.tabs.get(a, function(b) {
                        if (b) {
                            var c = tree.getNode(["chromeId", a]);
                            if (c && (!isStaticFavIconUrl(c.favicon) ) )
                                isStaticFavIconUrl(b.favIconUrl) && b.favIconUrl != c.favicon ? tree.updatePage(c, {
                                    favicon: b.favIconUrl
                                }) : tree.updatePage(c, {
                                    favicon: getChromeFavIconUrl(getUrl(c))
                                })
                        }
                    })
